...
function GalleryThumbnail({ rawUrl, alt, onClick }: GalleryThumbnailProps) {
  return (
    <div
...
export default function App() {
  ...
  useEffect(() => {
    // Natychmiastowe ładowanie wstępne kluczowych obrazów tła i logo
    const imagesToPreload = [HERO_BG, LOGO_URL, ...GALLERY_PHOTOS.map((p) => p.rawUrl)];
          ...
          >
            ...
            <div className="flex items-center justify-center max-h-[80vh] min-h-[260px] overflow-hidden bg-[#181E30]">
              <img
                ...
                fetchpriority="high"
                decoding="sync"
                className="max-h-[80vh] w-auto max-w-full object-contain opacity-100"
              />
            </div>
          </div>
        </div>
